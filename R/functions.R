...
pbssn<-function(q,alpha=0.5,beta=1,lambda=2)
{
  if(alpha<=0){stop("alpha must be positive")}
  if(beta<=0){stop("beta must be positive")}
...
#pbssn(c(25,3),0.75,1,3)

qbssn <- function(p,alpha=0.5,beta=1,lambda=2)
{
  if(alpha<=0){ stop("alpha must be positive") }
  if(beta<=0) { stop("beta must be positive")  }
...
#logLikbssn(c(1,2,4,5),alpha=1, beta=3, lambda=2)
